    ...
    <div className="flex items-center justify-between py-5  font-medium ">
      ...
      <div className="flex items-center gap-6 ">
        ...
        <Link to="./cart" className="relative">
          <img src={assets.cart_icon} alt="cart_icons" className="w-5 min-w-5" />
          <p className="absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px] ">{getCartCount()}</p>
        </Link>
        <img onClick={() => setVisible(true)} src={assets.menu_icon} alt="menu_icons" className="w-5 cursor-pointer sm:hidden " />
      </div>
      {/* Side bar Menu for Smaller Screen  */}
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all  ${visible ? 'w-full' : 'w-0'}`}>
        <div className="flex flex-col text-gray-600">
          <div onClick={() => setVisible(false)} className="flex items-center gap-4 p-3">
            <img src={assets.dropdown_icon} alt="drop_down_menu " className="h-4 rotate-180" />
            <Link to="./"><p>Back</p></Link>
          </div>
          {/* <NavLink onClick={()=>setVisible(false)} className="py-2 pl-6 border" to="/">HOME</NavLink> */}
          <NavLink onClick={() => setVisible(false)} className="py-2 pl-6 border" to="/mens">MENS</NavLink>
          <NavLink onClick={() => setVisible(false)} className="py-2 pl-6 border" to="/womens">WOMENS</NavLink>
          <NavLink onClick={() => setVisible(false)} className="py-2 pl-6 border" to="/kids">KIDS</NavLink>
          <NavLink onClick={() => setVisible(false)} className="py-2 pl-6 border" to="/accessories">LIFESTYLE ACCESSORIES</NavLink>
